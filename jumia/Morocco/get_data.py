...
url = 'https://www.jumia.ma/catalog/?q=televisions'

#%%
##create database
jumia_morocco_db = mysql.connector.connect(
  host="localhost",
  user="root",
  password="4156"
)

mycursor = jumia_morocco_db.cursor()

mycursor.execute("use jumia_morocco_db")
mycursor.execute("""CREATE TABLE IF NOT EXISTS tv_links(tv_link varchar(200) UNIQUE, is_scraped TINYINT)""")

# Credentials to database connection
hostname="localhost"
dbname="jumia_morocco_db"
uname="root"
pwd="4156"
...
df_long = df_l.set_index(['countrycode', 'category1','category2','tv_title', 'tv_make', 'Modèle','tv_price', 'numberofoffers', 'averageprice', 'currency', 'scrapelink', 'scrapedate']).stack().reset_index()
