...
#%%
##create database
mercado_db_ecuador = mysql.connector.connect(
  host="localhost",
  user="root",
  password="4156",
...
mycursor = mercado_db_ecuador.cursor()

# Credentials to database connection
hostname="localhost"
dbname="mercado_db_ecuador"
uname="root"
pwd="4156"
...
mycursor.execute("DROP DATABASE  IF EXISTS mercado_db_ecuador")
mycursor.execute("CREATE DATABASE  IF NOT EXISTS mercado_db_ecuador")
mycursor.execute("use mercado_db_ecuador")
mycursor.execute("""CREATE TABLE IF NOT EXISTS tv_links(link varchar(200) UNIQUE, is_scraped TINYINT)""")

#%%
link = "https://listado.mercadolibre.com.ec/tv#D[A:TV]"
is_scraped = 0
def get_links(link):
