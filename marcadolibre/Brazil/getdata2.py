...
#%%
##create database
mercado_db_brazil = mysql.connector.connect(
  host="localhost",
  user="root",
  password="4156",
...
mycursor = mercado_db_brazil.cursor()

# Credentials to database connection
hostname="localhost"
dbname="mercado_db_brazil"
uname="root"
pwd="4156"
...
mycursor.execute("use mercado_db_brazil")
...
#%%
df_links = pd.read_sql('SELECT * FROM tv_links', con=mercado_db_brazil)
...
#%%
async def get_data(s, link):
    from datetime import datetime
    time.sleep(1)
    r = await s.get(link)
    r.html.arender(sleep=5)
    time.sleep(1)
    hun = BeautifulSoup(r.html.raw_html, "html.parser")
...
async def main(links):
    s = AsyncHTMLSession()
    tasks = (get_data(s, link) for link in links)
...
df = df.astype(str)
df.to_sql('data_table2', con=engine, if_exists="append", index=False)



#%%
df = pd.read_sql('SELECT * FROM data_table', con=mercado_db_brazil)
##Feature Engineering
